...
import com.order.srm.exception.EntityExistException;
import com.order.srm.exception.EntityNotFoundException;
...
import com.order.srm.modules.system.repository.UserAvatarRepository;
import com.order.srm.modules.system.repository.UserRepository;
...
import com.order.srm.modules.system.service.dto.UserQueryCriteria;
import com.order.srm.modules.system.service.mapper.UserMapper;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Cacheable(key = "'loadUserByUsername:'+#p0 +'_' +#p1")
    public UserDto findByNameAndCode(String userName, String code) {
        User user;
        if (ValidationUtil.isEmail(userName)) {
            user = userRepository.findByEmailAndCompany_Code(userName, code);
        } else {
            user = userRepository.findByUsernameAndCompany_Code(userName, code);
        }
        if (user == null) {
            throw new EntityNotFoundException(User.class, "name", userName);
        } else {
